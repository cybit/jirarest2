...
stub_request(:put, "http://admin:admin@localhost:2990/jira/rest/api/2/issue/SP-2/assignee").with(:body => "null",
       :headers => {'Accept'=>'*/*', 'Content-Type'=>'application/json;charset=UTF-8', 'User-Agent'=>'Ruby'}).to_return(:status => 204, :body => "", :headers => {})
    cred = Credentials.new("http://localhost:2990/jira/rest/api/2/","admin","admin")
    con = Connect.new(cred)
    issue = Issue.new("SP-2")
